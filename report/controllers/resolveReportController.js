...
const { USER_STATUS_SUSPENDED, USER_STATUSES } = require('../../user/models/user.js');
...
async function reportFailed(req, res) {
    try {
        ...
        await Report.resolveReport(req.params.objectId, REPORT_STATUS_FAILED, adminId, now);
...
async function reportSuccess(req, res, type, reportTargetId, userAction) {
    try {
        ...
        const now = Date.now();
        const reviewReason = req.body.reason == REPORT_REASON_OTHER ? req.body.otherReason : req.body.reason;
        ...
        const promises = [
            Report.resolveReport(reportTargetId, REPORT_STATUS_SUCCESS, adminId, now, reviewReason)
        ];
