...
const REPORT_MESSAGE_SUBMITTED =
  "Report submitted successfully, please wait for the review by our moderation team.";

// valid reporter subject
const REPORTER_SUBJECT_AI = "AI";
const REPORTER_SUBJECT_USER = "User";

const REPORTER_SUBJECTS = [REPORTER_SUBJECT_AI, REPORTER_SUBJECT_USER];
...
reportSchema.virtual("report_message").get(function () {
  ...
  const REPORT_MESSAGE_REVIEWED_PREFIX =
    "Thank you for reporting. We have reviewed the";
  ...
  if (this.status == REPORT_STATUS_SUCCESS) {
    message += `appropriate action has been taken: ${
      this.report_target_type == REPORT_TARGET_TYPE_USER
        ? REPORT_MESSAGE_SUCCESS_USER
        : REPORT_MESSAGE_SUCCESS_OTHER
    }`;
  }
