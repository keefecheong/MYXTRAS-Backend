...
const { POST_USER_KEY_BASE, getUserPostKey, getPostLikesPath, getPostSavesPath } = require('../cache/postCache.js');
...
async function verifyDBUserReactedToUserPosts(reactingUserId, postOwnerId, blocked, forLikes) {
    const posts = await Post.find({ creator_id: postOwnerId }).lean();    
    var reacted = forLikes ? 
        posts.map(post => post.likes.some(likeId => compareId(likeId, reactingUserId))) :
        posts.map(post => post.saved_by.some(saveId => compareId(saveId, reactingUserId)));
...
async function verifyCacheUserReactedToUserPosts(reactingUserId, postOwnerId, blocked, forLikes) {
    const path = forLikes ? getPostLikesPath(reactingUserId) : getPostSavesPath(reactingUserId);
    const reacted = await redisClient.json.get(getUserPostKey(postOwnerId), { path });
...
async function verifyDBUserReactedToOtherPosts(userId, terminated, forLikes) {
    ...
    forLikes ? filter.likes = reactionFilter : filter.saved_by = reactionFilter;
...
async function verifyCacheUserReactedToOtherPosts(userId, terminated, forLikes) {
    var reacted = [];
