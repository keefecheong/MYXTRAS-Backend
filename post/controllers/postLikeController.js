...
} = require("../cache/postLikeCache.js");
...
async function postLike(req, res) {
  ...
  post = new Post(res.post);
  post.isNew = false;

  // check if the specified post is liked by the user
  const likeExists = post.likes.find((creator_id) =>
    compareId(creator_id, userId),
  );
  ...
  try {
    // update cache entry
    const updateCacheResult = await cachedPostAddLike(
      post.creator_id._id,
      post._id,
      userId,
    );
...
async function deleteLike(req, res) {
  ...
  // check if the specified post is liked by the user
  const likeIndex = res.post.likes.findIndex((user_id) =>
  ...
  try {
    // update cache entry
    const updateCacheResult = await cachedPostRemoveLike(
      post.creator_id._id,
      post._id,
      likeIndex,
    );
