...
module.exports = async function addThreadsToDB(
  count,
  creatorIds,
  forumIds,
  commentCount,
  ...
  const threads = forumIds.flatMap((forumId) =>
    creatorIds.flatMap((creatorId) =>
      generateThreadsPerCreatorAndForum(
        count,
        forumId,
        creatorId,
        creatorIds,
  ...
  const threadComments = await addCommentsToDB(
    commentCount,
...
function generateThreadsPerCreatorAndForum(
  count,
  parentId,
  creatorId,
  userIds,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const likes = userIds.filter(
      (userId, index) =>
        (i % 2 == 0 && index % 2 == 0) || (i % 2 != 0 && index % 2 != 0),
    );
    // dislikes cannot have repeated user ids in likes
    const dislikes = userIds.filter(
      (userId) => !likes.some((likedId) => compareId(likedId, userId)),
    );
...
async function populateThreadCommentsCache(threads, userId) {
  await Promise.all(
    threads.map((thread) =>
      sendMockRequest(
        `/api/threads/forum/${thread.parent_id}/thread/${thread._id}/comments`,
        userId,
        "get",
      ),
    ),
