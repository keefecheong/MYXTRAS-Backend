...
const { deleteThread, verifyDBDeleteThread, verifyCacheDeleteThread } = require('./verifyDeleteThread.js');
...
let mainSuite = describe('DELETE /api/threads/forum/:forumId/thread/:threadId', () => {
    before(async () => {
...
function generateDeleteThreadTestContext(deleted) {
    return generateContextFromTests(deleted, 'deleting', generateDeleteThreadTests(deleted));
}
