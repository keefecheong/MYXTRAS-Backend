...
const { USER_SINGLE_KEY_BASE, getUserKey, getBlockedPath } = require('../cache/userCache.js');
...
async function blockUser(blockerId, blockedId) {
    const res = await sendMockRequest(`/api/users/${blockedId}/block`, blockerId, 'post');
...
async function verifyDBBlockUser(blockerId, blockedId, blocked) {
    const blockedEntryExists = (await User.findById(blockerId).lean()).blocked_users.some(entry => compareId(entry.user_id, blockedId));
...
async function verifyCacheBlockUser(blockerId, blockedId, blocked) {
    expectEmpty(await redisClient.json.get(getUserKey(blockerId), { path: getBlockedPath(blockedId) }), !blocked);
}
...
async function verifyCacheUserIsBlockingAny(userId, terminated) {
    expectEmpty((await redisClient.json.get(getUserKey(userId), { path: '$.blocked_users' })).flat(), terminated);
}
...
async function verifyDBUserIsBlockedByAny(userId, terminated) {
    expectEmpty(await User.find({ 'blocked_users.user_id': { $in: [userId] } }), terminated);
}
