...
async function verifyDBFollowing(followingUserId, followedUserId, blocked) {
  expect(
    (await User.findById(followedUserId).lean()).followers.some((followerId) =>
...
async function verifyCacheFollowing(followingUserId, followedUserId, blocked) {
  expectEmpty(
    await redisClient.json.get(getUserKey(followedUserId), {
...
async function verifyCacheHasFollowers(userId, terminated) {
  expectEmpty(
    (
      await redisClient.json.get(getUserKey(userId), { path: "$.followers" })
    ).flat(),
    terminated,
  );
}
...
async function verifyDBIsFollowingAny(userId, terminated) {
  expectEmpty(
    await User.find({ followers: { $in: userId } }).lean(),
    terminated,
  );
}
