...
} = require("./verifyBlockUser.js");
const {
  verifyDBFollowing,
  verifyCacheFollowing,
} = require("./verifyUserFollowers.js");
const {
...
function generateBlockUserTestContext(blocked) {
  ...
  suite.suites = [
    generateContextFromTests(
      null,
      null,
      generateBlockUserTests(blocked, true),
      "for blocking user",
    ),
    generateContextFromTests(
      null,
      null,
      generateBlockUserTests(blocked, false),
      "for blocked user",
    ),
  ];
...
function generateBlockUserTests(blocked, forBlockingUser) {
  ...
  const tests = [
    ...
    // check if userId1 has created any comments under posts created by userId2
    {
      title: generateTestTitle(
        blocked,
        `comments on ${otherUserName}' posts`,
        true,
      ),
