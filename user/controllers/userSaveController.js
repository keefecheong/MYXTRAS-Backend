...
const schools = require("../../school/utils/schools.json");
const tasks = require("../../gamification/utils/config.json");

const { updateCachedUser } = require("../cache/userUpdateCache.js");
...
async function updateUser(req, res) {
  ...
  const { userName, biography, selectedInterests, gender } = JSON.parse(
    req.body.userObject,
  );
...
async function setupUser(req, res) {
  ...
  try {
    ...
    if (realName.length > 32) {
      return returnBadReq(
        res,
        "Real name must not be more than 32 characters long",
      );
    }
    ...
    if (userName.length > 16) {
      return returnBadReq(
        res,
        "Username must not be more than 16 characters long",
      );
    }
    ...
    if (
      !Object.keys(schools[selectedSchool]["courses"]).includes(selectedCourse)
    ) {
