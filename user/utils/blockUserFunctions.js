...
async function handleBlockPerUser(self, targetUserId, isBlocker) {
  ...
  const followerIndex = user.followers.findIndex((user_id) =>
    compareId(user_id, targetUserId),
  );
  ...
  const removePostLikes = Post.removePostReactionByUser(
    user._id,
    true,
    targetUserId,
  );
  ...
  const removeSave = Post.removePostReactionByUser(
    user._id,
    false,
    targetUserId,
  );
  ...
  const deleteComments = Comment.deleteAllSpecified(
    postIds,
    targetUserId,
    true,
  );
  ...
  // update cache
  const cachePromises = await cachedUserAddBlocked(
    self._id,
    isBlocker,
    blockEntry,
    commentsPerPost,
