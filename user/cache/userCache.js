...
async function getUserFromCache(key, populateFollowers) {
    ...
    if (populateFollowers && user?.followers.length > 0) {
        ...
        user.followers = await Promise.all(user.followers.map(followerId => redisClient.json.get(getHeaderKey(followerId))));
    }
...
function cacheUser(data, key, populateFollowers) {
    ...
    const workingData = populateFollowers ? JSON.parse(JSON.stringify(data)) : data;
    ...
    if (followers?.length > 0) {
        followers.forEach(follower => {
            ...
            promises.push(redisClient.expire(followerHeaderKey, USER_EXPIRATION_TIME));
        });
    }
