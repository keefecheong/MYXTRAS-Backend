...
const { getHeaderKey } = require("../utils/cacheProperties.js");
...
} = require("../../post/cache/postLikeCache.js");
const {
  cachedPostRemoveSaveByUser,
} = require("../../post/cache/postSaveCache.js");
...
async function cachedUserAddBlocked(
  selfId,
  isBlocker,
  blockEntry,
  commentsPerPost,
  ...
  if (isBlocker) {
    promises.push(
      redisClient.json.arrAppend(selfKey, "$.blocked_users", blockEntry)
    );
    promises.push(
  ...
  commentsPerPost.forEach((entry) => {
    promises.push(
      deleteAllCachedComments(entry._id, PARENT_MODEL_POST, targetUserId)
    );
