...
async function terminateUser(terminatedUserId, adminId) {
  const res = await sendMockRequest(
    `/api/admin/accounts/terminate/${terminatedUserId}`,
    adminId,
    "post",
  );
...
async function verifyDBUserTerminated(userId, terminated) {
  expectEqualValue(
    (await User.findById(userId).lean()).status?.status,
    terminated ? USER_STATUS_TERMINATED : undefined,
  );
}
...
async function verifyCacheUserTerminated(userId, terminated) {
  expectEqualValue(
    await redisClient.json.get(getUserKey(userId), { path: "$.status.status" }),
    terminated ? USER_STATUS_TERMINATED : null,
  );
}
