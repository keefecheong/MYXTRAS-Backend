...
} = require("./verifyTerminateUser.js");
...
} = require("../../comment/test/verifyCommentByUser.js");

const {
  verifyDBTerminatedUserChats,
} = require("../../chat/test/verifyTerminatedUserChats.js");
...
let terminatedUserId,
  adminId,
  terminatedUserPostIds,
  terminatedUserForumIds,
  terminatedUserThreadIds;

let mainSuite = describe("POST /api/admin/accounts/terminate/:userId", () => {
  before(async () => {
    const userIds = (await addUsersToDB(userCount, true)).map(
      (user) => user._id,
    );
    ...
    terminatedUserPostIds = posts
      .filter((post) => compareId(post.creator_id, terminatedUserId))
      .map((post) => post._id);
    ...
    terminatedUserForumIds = forums
      .filter((forum) => compareId(forum.creator_id, terminatedUserId))
      .map((forum) => forum._id);
    ...
    terminatedUserThreadIds = threads
      .filter((thread) => compareId(thread.creator_id, terminatedUserId))
...
function generateTerminateUserTestContext(terminated) {
  return generateContextFromTests(
    terminated,
    "terminating",
    generateTerminateUserTests(terminated),
  );
}
...
function generateTerminateUserTests(terminated) {
  ...
  const tests = [
    ...
    // check if terminated user has created posts in database and cache
    {
      title: generateTestTitle(
        terminated,
        "posts created by terminated user",
        true,
      ),
    ...
    // check if terminated user has created forums in database and cache
    {
      title: generateTestTitle(
        terminated,
        "forums created by terminated user",
        true,
      ),
    ...
    // check if terminated user has created threads in database and cache
    {
      title: generateTestTitle(
        terminated,
        "threads created by terminated user",
        true,
      ),
    ...
    // check if terminated user has created comments in database and cache
    {
      title: generateTestTitle(
        terminated,
        "comments created by terminated user",
        true,
      ),
    ...
    // check if terminated user has chats and messages in database
    {
      title: generateTestTitle(
        terminated,
        "chats and messages created by terminated user",
        true,
      ),
    ...
    // check if threads exist under the terminated user's forums in database and cache
    {
      title: generateTestTitle(
        terminated,
        "threads under forums created by terminated user",
        true,
      ),
    ...
    // check if comments exist under the terminated user's posts in database and cache
    {
      title: generateTestTitle(
        terminated,
        "comments under posts created by terminated user",
        true,
      ),
    ...
    // check if comments exist under the terminated user's threads in database and cache
    {
      title: generateTestTitle(
        terminated,
        "comments under threads created by terminated user",
        true,
      ),
      callback: verifyDBDeleteCommentParent,
    ...
    // check if terminated user has liked posts by other users
    {
      title: generateTestTitle(
        terminated,
        "liked posts created by other users",
        true,
        "have",
      ),
    ...
    // check if terminated user has saved posts by other users
    {
      title: generateTestTitle(
        terminated,
        "saved posts created by other users",
        true,
        "have",
      ),
      callback: verifyDBUserReactedToOtherPosts,
    ...
    // check if terminated user has subscribed to forums by other users
    {
      title: generateTestTitle(
        terminated,
        "subscribed to forums created by other users",
        true,
        "have",
      ),
    ...
    // check if terminated user has liked threads by other users
    {
      title: generateTestTitle(
        terminated,
        "liked threads created by other users",
        true,
        "have",
      ),
    ...
    // check if terminated user has disliked threads by other users
    {
      title: generateTestTitle(
        terminated,
        "disliked threads created by other users",
        true,
        "have",
      ),
      callback: verifyDBUserReactedToOtherThreads,
