...
// get middleware
const { validateUserSocket } = require("../../middleware/authMiddleware.js");

// get controller functions
const {
  handleNewConnection,
  handleDisconnection,
} = require("../controllers/chatSocketConnectionController.js");
const {
  handleSendMessage,
  handleEditMessage,
  handleDeleteMessage,
} = require("../controllers/chatSocketMessageController.js");
const {
  handleUserTyping,
  handleQueryUserPresence,
} = require("../controllers/chatSocketStatusController.js");
...
function chatHandler(io) {
  ...
  chatNamespace.on("connection", (socket) => {
    ...
    socket.on("send-message", (data) =>
      handleSendMessage(data, socket, connections),
    );

    // handle edit message
    socket.on("edit-message", (data) =>
      handleEditMessage(data, socket, connections),
    );

    // handle delete message
    socket.on("delete-message", (data) =>
      handleDeleteMessage(data, socket, connections),
    );

    // get user's presence (online/offline)
    socket.on("query-user-presence", (data, callback) =>
      handleQueryUserPresence(data, callback, socket, connections),
    );
