...
async function handleSendMessage(data, socket, connections) {
  ...
  emitSocketEvent(socket, "self", "receive-message", {
  ...
  if (getUserOnline(connections, data.chat.targetUserId)) {
    ...
    if (recipientMessage.reply_message) {
      recipientMessage.reply_message.is_sender =
        !recipientMessage.reply_message.is_sender;
    }
    ...
    emitSocketEvent(socket, data.chat.targetUserId, "receive-message", {
      message: recipientMessage,
      chat: recipientChat,
    });

    // update typing status to false immediately
    emitSocketEvent(socket, data.chat.targetUserId, "receive-user-typing", {
  ...
  Chat.findById(data.chat._id).then((chat) => {
    ...
    if (!chat) {
      chat = new Chat({
        _id: data.chat._id,
        users: [data.chat.targetUserId, socket.user._id],
      });
  ...
  updateUserTasks(socket.user, "Message a friend");
}
