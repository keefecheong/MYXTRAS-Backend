...
async function verifyDBTerminatedUserChats(terminatedUserId, terminated) {
  const enrolledChats = await Chat.find({
    users: { $in: terminatedUserId },
  }).lean();
  ...
  expectEmpty(
    messages.filter((message) =>
      enrolledChatIds.some((chatId) => compareId(chatId, message.chat_id)),
    ),
    terminated,
  );
  ...
  expectEmpty(
    messages.filter((message) =>
      compareId(message.creator_id, terminatedUserId),
    ),
    terminated,
  );
}
