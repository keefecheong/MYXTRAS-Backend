...
const { handleBlockPerUser } = require('../../utils/users/blockUserFunctions.js');

const returnGoodReq = require('../../utils/general/returnGoodReq.js');
const returnBadReq = require('../../utils/general/returnBadReq');
const returnServerErrorReq = require('../../utils/general/returnServerErrorReq');

const { cachedUserRemoveBlocked } = require('../../cache/users/userBlockCache.js');
const { cacheUnblock } = require('../../cache/unblock/unblockCache.js');
...
async function blockUser(req, res) {
    ...
    if (self.blocked_users.some(entry => compareId(entry.user_id, targetUser._id))) {
        return returnBadReq(res, 'You have already blocked this user.');
    }

    try {
        const recentUnblock = await Unblock.findOne({ blocker_id: self._id, blocked_id: targetUser._id }, { unblock_time: 1 }).cache();

        // if recentUnblock document exists for this set of users means the minimum time before reblocking has not passed
        if (recentUnblock) {
            return returnBadReq(res, `User can be blocked again in ${calcTimeDifference(recentUnblock.unblock_time, UNBLOCK_TTL)}.`);
        }
...
async function unblockUser(req, res) {
    ...
    const blockIndex = self.blocked_users.findIndex(entry => compareId(entry.user_id, targetUser._id));
    ...
    if (!unblockAvailable) {
        return returnBadReq(res, `You can unblock this user in ${calcTimeDifference(blockTime, UNBLOCK_TTL)}.`);
    }
