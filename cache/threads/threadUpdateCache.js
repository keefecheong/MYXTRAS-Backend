...
async function cacheNewThread(thread, userDetails, forumDetails) {
    ...
    return await returnPromiseResult(redisClient.json.arrInsert(forumThreadKey, '$', 0, jsonThread));
}

// to update thread data in cache if exists
async function updateCachedThread(updatedValues, forumId, threadId, threadFromCache) {
    ...
    const promises = [redisClient.json.numIncrBy(forumThreadKey, `${threadPath}.__v`, 1)];
    
    // add promise for each updated key/value
    for (const updatedKey in updatedValues) {
        if (updatedValues.hasOwnProperty(updatedKey)) {
            promises.push(redisClient.json.set(forumThreadKey, `${threadPath}.${updatedKey}`, updatedValues[updatedKey]));
        }
    }
