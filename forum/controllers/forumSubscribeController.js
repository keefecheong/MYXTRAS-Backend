...
} = require("../cache/forumSubscribeCache.js");
...
async function subscribeToForum(req, res) {
  try {
    ...
    const subscribed = res.forum.subscribers.find((creator_id) =>
...
async function unsubscribeFromForum(req, res) {
  try {
    ...
    // check if the requesting user has subscribed to the forum already
    const subscriberIndex = res.forum.subscribers.findIndex((creator_id) =>
      compareId(creator_id, userId),
    );
    ...
    // update cached forum
    const updateCacheResult = await cachedForumRemoveSubscriber(
      forum._id,
      userId,
    );
