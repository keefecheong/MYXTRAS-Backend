...
const { REPORT_TARGET_TYPE_FORUM } = require("../../report/models/report.js");
...
const {
  cacheNewForum,
  updateCachedForum,
} = require("../cache/forumUpdateCache.js");
...
async function createForum(req, res) {
  ...
  try {
    ...
    if (
      forum_name.length > 25 ||
      forum_id.length > 25 ||
      forum_desc.length > 100
    ) {
    ...
    // upload images
    const imageLinks = [];
    const forumPicUploadSuccessful = await uploadImages(
      req.files,
      imageLinks,
      newForum._id,
      UPLOAD_TYPE_FORUM
...
async function updateForum(req, res) {
  ...
  if (
    req.files.length <= 0 &&
    req.body.pictureUnchanged != "true" &&
    req.body.bannerUnchanged != "true"
  ) {
