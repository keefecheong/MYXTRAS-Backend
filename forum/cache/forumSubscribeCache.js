...
async function cachedForumAddSubscriber(forumDetails, userId) {
  ...
  if (subscribedEntryExists) {
    promises.push(
      redisClient.json.arrAppend(subscribedForumKey, "$", forumDetails),
    );
  }
...
async function cachedForumRemoveSubscriber(forumId, userId, forTerminate) {
  ...
  const removeSubscriberPromise = redisClient.json.del(
    forumKey,
    getForumSubscriberPath(userId),
  );
  ...
  const increaseVersionPromise = redisClient.json.numIncrBy(
    forumKey,
    "$.__v",
    1,
  );
  ...
  const subscribedForumKey = getSubscribedForumKey(userId);
  const removeSubscribedForumPromise = redisClient.json.del(
    subscribedForumKey,
    getForumIdPath(forumId),
  );

  return await returnPromiseResult([
    removeSubscriberPromise,
    increaseVersionPromise,
    removeSubscribedForumPromise,
  ]);
}
