...
const { getPostCommentKey, getCommentByUserPath } = require('../cache/commentCache.js');
...
async function verifyCommentedOnUserPost(commentOwnerId, postOwnerId, blocked, forDatabase) {
    const postIds = (await Post.find({ creator_id: postOwnerId }).lean()).map(post => post._id);
    const comments = forDatabase ? 
        await Comment.find({ creator_id: commentOwnerId, parent_id: { $in: postIds } }).lean() :
        (await Promise.all(postIds.map(postId => redisClient.json.get(getPostCommentKey(postId), { path: getCommentByUserPath(commentOwnerId) })))).flat();
...
async function verifyCacheUserHasCreatedComments(userId, terminated) {
    ...
    for await (const key of redisClient.scanIterator({ MATCH: 'comment:*' })) {
        comments.push(...(await redisClient.json.get(key, { path: getCommentByUserPath(userId) })));
    }
