...
} = require("./commentCache.js");
const {
  getPostIdPath,
  getUserPostKey,
} = require("../../post/cache/postCache.js");
const {
  getThreadIdPath,
  getForumThreadKey,
...
function deleteCachedComment(
  ...
  if (parentInCache) {
    const parentPath = forPost
      ? getPostIdPath(parentId)
      : getThreadIdPath(parentId);
    promises.push(
      redisClient.json.numIncrBy(parentKey, `${parentPath}.comment_count`, -1),
    );
  }
...
function updateCachedParentCommentCount(
  keyCreationId,
  parentId,
  parentModel,
  deletedCommentCount,
